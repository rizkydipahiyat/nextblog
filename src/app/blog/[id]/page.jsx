...
const BlogDetail = (ctx) => {
  ...
  const handleDelete = async () => {
    try {
      const confirmModal = confirm("Are you sure want to delete this post?");
      if (confirmModal) {
        const res = await fetch(`${process.env.NEXTAUTH_URL}/api/blog/${id}`, {
          headers: {
  ...
  const handleLike = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXTAUTH_URL}/api/blog/${id}/like`,
        {
          headers: {
            Authorization: `Bearer ${session?.user?.accessToken}`,
          },
    ...
    <div className={classes.container}>
      <div className={classes.wrapper}>
        ...
        <div className={classes.row}>
          <h3 className={classes.title}>{blogDetail?.title}</h3>
        ...
        <div className={classes.row}>
          <p>{blogDetail?.desc}</p>
          <span>
            Posted: <span>{format(blogDetail?.createdAt)}</span>
          </span>
        </div>
